...
def print_patient_feedback(
    ...
    Parameters
    ----------
    velocity_buffer_pct : float
        Percentage of template length used as a ±buffer for velocity
        lead/lag tolerance (e.g. 0.10 = ±10%).
    template_frame_count : int
        Total number of frames in the reference template (used to
        compute the buffer in frames).
    """
    ref = metrics["Reference"]
    pat = metrics["Patient"]
    ...
    #    Buffer is derived from template length * velocity_buffer_pct
